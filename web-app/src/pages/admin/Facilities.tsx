...
export default function FacilitiesLocations() {
  ...
  const fetchLocations = useCallback(async () => {
    try {
      ...
      const locs = (data.facilities || data.locations || []).map((f: any) => ({
        id: f.id?.slice(0, 8) || f.id,
        fullId: f.id,
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-medium text-slate-500 mb-1">{loc.facility} • {loc.id}</p>
          ...
          <div className="p-4 border-b border-slate-200 bg-slate-50/50 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">{selectedLocation.facility} • {selectedLocation.id}</p>
              </div>
            </div>
            <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded border ${
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-6 grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-slate-50 rounded-xl border border-slate-200 p-4">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button onClick={async () => {
                    const newRadius = prompt('Enter new geofence radius (50-1000m):', '200');
                    if (newRadius) {
                      ...
                      try {
                        ...
                        fetchLocations();
                      } catch (err: any) { toast.error('Failed to update', err.message); }
                    }
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-mono text-slate-900">31.5204° N, 74.3587° E</p>
                  </div>
                  <div>
                    <p className="text-xs text-slate-500 font-medium mb-1">Geofence Radius</p>
                    <div className="flex items-center gap-3">
                      <input type="range" className="flex-1 accent-indigo-600" min="50" max="1000" defaultValue="200" />
                      <span className="text-sm font-bold text-indigo-600 w-12 text-right">{selectedLocation.geofence}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-xl border border-slate-200 p-4">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button onClick={async () => {
                    const newInterval = prompt('Enter QR rotation interval in minutes (5, 10, 15, or 30):', '15');
                    if (newInterval) {
                      ...
                      try {
                        ...
                        fetchLocations();
                      } catch (err: any) { toast.error('Failed to update: ' + (err.message || 'Unknown error')); }
                    }
                  }} className="text-xs font-medium text-indigo-600 hover:text-indigo-700 flex items-center gap-1">
                    <Edit3 className="w-3 h-3" /> Edit
                  </button>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <select className="p-2 text-sm border border-slate-200 rounded-lg bg-slate-50 text-slate-900 outline-none">
                      <option>5 mins</option>
                      <option>10 mins</option>
                      <option selected>15 mins</option>
                      <option>30 mins</option>
                      <option>Static (Not recommended)</option>
                    </select>
                  </div>
                  <div className="flex items-center justify-between">
                    <div>
                    ...
                    <div className="w-10 h-5 bg-indigo-600 rounded-full relative">
                      <div className="absolute top-0.5 left-5 w-4 h-4 bg-white rounded-full"></div>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
                <div className="p-4 border-b border-slate-200 bg-slate-50/50 flex items-center justify-between">
                  <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
                    <Clock className="w-4 h-4 text-slate-400" /> Rotation Log
                  </h3>
                </div>
                <div className="divide-y divide-slate-100 max-h-64 overflow-y-auto">
                  {[1, 2, 3, 4, 5].map((i) => (
                    <div key={i} className="p-3 flex items-center justify-between hover:bg-slate-50 transition-colors text-sm">
                      <div className="flex items-center gap-3">
                        <ShieldCheck className="w-4 h-4 text-emerald-500" />
                        <span className="text-slate-700">Auto-rotated</span>
                      </div>
                      <span className="text-slate-500 text-xs">{i * 15} mins ago</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 text-center">
              ...
              <div className="flex gap-3">
                ...
                <button 
                  onClick={() => setQrModalOpen(false)}
                  className="flex-1 py-3 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 transition-colors shadow-sm shadow-indigo-600/20"
                >
                  Generate
                </button>
              </div>
