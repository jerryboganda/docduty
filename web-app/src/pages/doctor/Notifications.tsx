...
export default function DoctorNotifications() {
  ...
  const handleClick = (n: Notification) => {
    ...
    try {
      ...
      if (data?.shiftId) navigate(`/doctor/shifts/${data.shiftId}`);
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors ${
              filter === 'unread' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'
            }`}
          >
        ...
        {loading && notifications.length === 0 ? (
          ...
          notifications.map(n => (
            <div
              ...
              className={`flex items-start gap-3 px-4 py-3 cursor-pointer transition-colors hover:bg-slate-50 ${!n.is_read ? 'bg-emerald-50/30' : ''}`}
            >
              <div className="pt-1.5 shrink-0">
                {!n.is_read ? <div className="w-2 h-2 bg-emerald-500 rounded-full" /> : <div className="w-2 h-2" />}
              </div>
